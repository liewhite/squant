...
class QuoteStyleSpec extends munit.FunSuite:
  private val bbo = Some(BBO(Exchange.Okx, "ETH", 2999.0, 1.0, 3001.0, 1.0, 0))

  test("被动: 卖挂 ask 之上、买挂 bid 之下 (PostOnly 不会被拒)"):
    val s = QuoteStyle.passive(0.01, 60_000)
    assertEquals(s.tif, TimeInForce.PostOnly)
    assertEquals(s.limitPrice(Side.Short, bbo, Price(3000.0)), Price(3001.0 * 1.01))
    assertEquals(s.limitPrice(Side.Long, bbo, Price(3000.0)), Price(2999.0 * 0.99))
  ...
  test("跨价: 卖挂 bid 之下、买挂 ask 之上 (到达即成交)"):
    ...
    assertEquals(s.limitPrice(Side.Short, bbo, Price(3000.0)), Price(2999.0 * 0.999))
    assertEquals(s.limitPrice(Side.Long, bbo, Price(3000.0)), Price(3001.0 * 1.001))

  test("两种方式的卖价方向相反 —— 这正是它们的区别所在"):
    val sell = Side.Short
    val passive = QuoteStyle.passive(0.01, 60_000).limitPrice(sell, bbo, Price(3000.0))
    val cross = QuoteStyle.crossing(0.001, 1000).limitPrice(sell, bbo, Price(3000.0))
  ...
  test("盘口缺失 -> 回退给定基准价 (降级路径, 调用方须告警)"):
    assertEquals(QuoteStyle.passive(0.01, 1000).limitPrice(Side.Short, None, Price(3000.0)), Price(3000.0 * 1.01))
    assertEquals(QuoteStyle.crossing(0.001, 1000).limitPrice(Side.Short, None, Price(3000.0)), Price(3000.0 * 0.999))

  test("零偏移合法 (挂在盘口上), 负偏移与非正 TTL 抛错"):
    assertEquals(QuoteStyle.passive(0.0, 1000).limitPrice(Side.Short, bbo, Price(3000.0)), Price(3001.0))
