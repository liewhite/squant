...
class QuoteLegSpec extends munit.FunSuite:
  ...
  private def upd(status: OrderStatus, px: Price = 3000.0, ts: Timestamp = 1000, oid: OrderId = "o1") =
    OrderUpdate(AccountId.Live, oid, Some("c1"), ex, sym, Side.Short, status, px, 1.0, 0.0, false, ts)
  ...
  test("place 后进入等确认 -> Blocked (一次穿越不会挂出两张单)"):
    ...
    leg.place(passive, Side.Short, bbo(2999.0, 3001.0))
  ...
  test("被动价在盘口外侧: 卖挂 ask 之上、买挂 bid 之下 (PostOnly 不吃单)"):
    val leg = QuoteLeg(cancelConfirmMs = 3000)
    val b = bbo(2999.0, 3001.0)
    assertEquals(leg.place(passive, Side.Short, b), (Price(3001.0 * 1.01), TimeInForce.PostOnly))
    val leg2 = QuoteLeg(cancelConfirmMs = 3000)
    assertEquals(leg2.place(passive, Side.Long, b), (Price(2999.0 * 0.99), TimeInForce.PostOnly))
  ...
  test("非空闲态调用 place -> 抛错 (绕过 step 就是在制造孤儿单)"):
    val leg = QuoteLeg(cancelConfirmMs = 3000)
    leg.place(passive, Side.Short, bbo(2999.0, 3001.0))
    intercept[IllegalArgumentException](leg.place(passive, Side.Short, bbo(2999.0, 3001.0)))
  ...
  test("反向不抢占: 在簿上的跨价单遇到被动需求 -> 等它自己到点"):
    ...
    val leg = QuoteLeg(cancelConfirmMs = 3000)
    leg.place(crossing, Side.Short, bbo(2999.0, 3001.0))
    feed(leg, upd(OrderStatus.Pending, ts = 0))
