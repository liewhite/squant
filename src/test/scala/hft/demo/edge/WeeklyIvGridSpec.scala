...
class WeeklyIvGridSpec extends munit.FunSuite:
  ...
  test("首周: IV=种子, 倍数=1.0 (不读任何 RV)"):
    val p = WeeklyIvGrid.planWeek(0, seed, _ => fail("不应读取 RV"), 1.5, 0.75)
    near(p.iv, seed); near(p.ivPrev, seed); near(p.mult, 1.0)

  test("次周: IV=上周RV, ivPrev=种子; 波动降→1.5×, 升→0.75×, 平→1.0×"):
    near(WeeklyIvGrid.planWeek(1, seed, _ => 0.50, 1.5, 0.75).mult, 1.5) // rv(0)=0.50<0.55 降
    near(WeeklyIvGrid.planWeek(1, seed, _ => 0.60, 1.5, 0.75).mult, 0.75) // 0.60>0.55 升
    near(WeeklyIvGrid.planWeek(1, seed, _ => 0.55, 1.5, 0.75).mult, 1.0)  // 持平
    val p = WeeklyIvGrid.planWeek(1, seed, _ => 0.50, 1.5, 0.75)
  ...
  test("第3周起: IV=上周RV, ivPrev=上上周RV"):
    ...
    val p = WeeklyIvGrid.planWeek(2, seed, rv, 1.5, 0.75)
  ...
  test("无前视: planWeek(idx) 只读 rv(j) 且 j<=idx-1"):
    ...
    def guarded(idx: Int): Int => Double = j =>
      ...
      0.5 + j * 0.01
    (0 to 8).foreach(i => WeeklyIvGrid.planWeek(i, seed, guarded(i), 1.5, 0.75))
