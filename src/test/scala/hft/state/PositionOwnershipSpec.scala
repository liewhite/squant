...
class PositionOwnershipSpec extends munit.FunSuite:
  ...
  private class Recorder(atFill: ConcurrentLinkedQueue[Double], atPosition: ConcurrentLinkedQueue[Double]) extends Strategy:
    ...
    def handlers = StrategyHandlers.empty
      ...
      .own(Topics.Fill) { (_, ctx, _) => atFill.add(ctx.state.instrumentState(Instrument.perp(ex, sym)).get.positionSize.value); Vector.empty }
      .own(Topics.Position) { (_, ctx, _) => atPosition.add(ctx.state.instrumentState(Instrument.perp(ex, sym)).get.positionSize.value); Vector.empty }
    ...
    supervised:
      ...
      class NoFill extends Strategy:
        ...
        def handlers = StrategyHandlers.empty.market(Topics.Bbo, inst) { (b, ctx, _) =>
          seen.add(ctx.state.instrumentState(Instrument.perp(ex, sym)).get.positionSize.value)
          if placed then Vector.empty
          else
            placed = true
            ctx.place(
  ...
  private class AcceptingClient extends TradingClient:
    ...
    override protected val metaFetchers = Map(InstrumentKind.LinearPerp -> (() => Right(Vector(metas(Instrument.perp(ex, sym))))))
        ...
        def handlers = StrategyHandlers.empty
          ...
          .own(Topics.OrderUpdate) { (u, ctx, _) =>
            if u.status.isTerminal then
              val view = ctx.state.instrumentState(Instrument.perp(ex, sym)).get
    ...
    supervised:
      ...
      class Peeker extends Strategy:
        def handlers = StrategyHandlers.empty.market(Topics.Bbo, inst) { (_, ctx, _) =>
          decisions.add(ctx.state.instrumentState(Instrument.perp(ex, sym)).get.positionSize.value)
          Vector.empty
        }
