...
class StateManagerSpec extends munit.FunSuite:
  ...
  test("USDT 余额更新，非 USDT 资产不影响"):
    val state = StateManager(List(Instrument.perp(Exchange.Binance, "BTCUSDT")), orderTimeoutMs = 5000)
  ...
  test("标的事件路由到对应 InstrumentState"):
    val state = StateManager(List(Instrument.perp(Exchange.Binance, "BTCUSDT"), Instrument.perp(Exchange.Binance, "ETHUSDT")), orderTimeoutMs = 5000)
    val bbo = BBO(Exchange.Binance, "ETHUSDT", 1600.0, Coin(1.0), 1600.1, Coin(1.0), t0)
    state.apply(Event.at(Topics.Bbo, bbo, t0))
    assertEquals(state.instrumentState(Instrument.perp(Exchange.Binance, "ETHUSDT")).flatMap(_.bbo), Some(bbo))
    assertEquals(state.instrumentState(Instrument.perp(Exchange.Binance, "BTCUSDT")).flatMap(_.bbo), None)

  test("Clock 事件驱动超时校验: 超时未确认订单 -> 抛错终止"):
    val state = StateManager(List(Instrument.perp(Exchange.Binance, "BTCUSDT")), orderTimeoutMs = 5000)
    state.addPendingOrder(newOrder("BTCUSDT", "c1"), t0) // createdAt = t0
    assert(state.hasPendingOrders(Instrument.perp(Exchange.Binance, "BTCUSDT")))
    // 未超时不抛
  ...
  test("未订阅标的的事件只可能是路由 bug -> 抛错终止"):
    ...
    val state = StateManager(Set(Instrument.perp(Exchange.Binance, "BTCUSDT")), orderTimeoutMs = 0L)
  ...
  test("私有回报按 AccountInstrument 路由, 仍要落到对应的 InstrumentState"):
    ...
    assertEqualsDouble(state.instrumentState(Instrument.perp(ex, "BTCUSDT")).get.positionSize.value, 2.0, 1e-12, "仓位快照必须落到 InstrumentState")
    ...
    assertEquals(
      state.instrumentState(Instrument.perp(ex, "BTCUSDT")).get.pendingOrders.head.order.id,
      "EX-9",
  ...
  test("行情仍按 Instrument 路由 (无账户维度), 同样要落到 InstrumentState"):
    ...
    assertEquals(state.instrumentState(Instrument.perp(ex, "BTCUSDT")).flatMap(_.bbo).map(_.bidPrice.value), Some(100.0))
  ...
  test("钱包快照到达前, greeks 返回 None —— 而不是把'没见过该币'当成 0"):
    ...
    val state = StateManager(List(Instrument.perp(Exchange.Okx, "ETH-USDT-SWAP")), orderTimeoutMs = 5000)
  ...
  test("钱包快照只清本交易所 —— 不抹掉另一家的余额"):
    val state = StateManager(
      List(Instrument.perp(Exchange.Okx, "ETH-USDT-SWAP"), Instrument.perp(Exchange.Bybit, "ETHUSDT")),
      orderTimeoutMs = 5000,
    )
    state.apply(Event.local(Topics.Greeks, Greeks(AccountId.Live, Exchange.Okx, "ETH", 1.0, 0.1, 0.0, 0.0, t0)))
