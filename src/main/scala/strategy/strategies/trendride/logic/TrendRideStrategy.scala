package strategy.strategies.trendride.logic
...
import strategy.strategies.trendride.logic.TrendRideLogic.{Desired, Exec, Params}
...
/** 趋势骑乘策略 (trade-native, taker 强制风控)。
  ...
  * BBO；maker 限价由真实成交价穿越成交。系数由回测标定，见 strategy.strategies.trendride.backtest.TrendRideBacktest。
  */
