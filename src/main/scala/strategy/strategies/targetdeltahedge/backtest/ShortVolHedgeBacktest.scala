package strategy.strategies.targetdeltahedge.backtest
import strategy.utils.backtest.ShortVolParams
import strategy.utils.backtest.ShortVolHedgeRunner

import hft.backtest.BsGreeksConfig
import strategy.strategies.targetdeltahedge.logic.{AdaptiveBandScaler, DeltaKamaFilter, RatchetSwitch, TargetDeltaHedgeStrategy}
import strategy.utils.hedge.{HedgeExecution, HedgeOverlay, KamaTrendOverlay, MacdBiasOverlay}
...
/** 卖方 (short-vol) + 方向性 **目标 delta** 对冲回测 ([[TargetDeltaHedgeStrategy]])。
  ...
  *
  * 运行: sbt "runMain strategy.strategies.targetdeltahedge.backtest.ShortVolHedgeBacktest [start] [end] [fee] [tilt] [band] [market|limit] [iv]
