package strategy.strategies.gammascalp.backtest
...
import strategy.strategies.gammascalp.logic.GammaScalpStrategy
...
/** 纯 gamma scalping 回测入口 ([[GammaScalpStrategy]])。
  ...
  *
  * 运行: sbt "runMain strategy.strategies.gammascalp.backtest.GammaScalpBacktest [START yyyy-MM-dd] [END yyyy-MM-dd]"
...
@main def GammaScalpBacktest(args: String*): Unit =
  ...
  // 单只 ATM 长跨式，到期日 = 回测结束日 (tenor ≈ 回测周期, 全程持有不滚动)；
  // 行权价由源在首笔成交时自设为当时价
  ...
  try
    ...
    // 完整 gamma scalp P&L = 期权腿 (单只跨式: 现价值 − 进场权利金) + 永续对冲腿。
    // 仅看对冲腿在趋势行情里会严重误导 (对冲腿亏损由期权腿盈利对冲)。
    val optionPnl = source.optionPnl(lastMid, lastTs)
    ...
    println(f"期权            : 起点ATM $atmStrike%.2f | IV=$impliedVol straddles=$straddles tenor=回测周期(持有到期) band=$deltaBand ETH")
