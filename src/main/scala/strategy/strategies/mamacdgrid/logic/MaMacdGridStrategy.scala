package strategy.strategies.mamacdgrid.logic
...
/** 均线 + MACD 方向性网格策略 (trade-native)。
  ...
  *
  * 系数 (档距/每档量/持仓上限) 由回测确定，见 strategy.strategies.mamacdgrid.backtest.MaMacdGridBacktest。
  */
final class MaMacdGridStrategy(
    ...
    /** 【方案2】价偏离均线超过 atrStretchN×ATR 时进入移动止盈 且 停止该方向加仓 (0=关闭)。
      * N 由历史 |price−ma|/atr 分布定 (见 strategy.strategies.mamacdgrid.backtest.MaDeviationAnalysis)，度量趋势拉伸/超买超卖。 */
