package strategy.targetdeltahedge.backtest
...
import strategy.targetdeltahedge.logic.{AdaptiveBandScaler, DeltaKamaFilter, RatchetSwitch, TargetDeltaHedgeStrategy}
...
/** 卖方 (short-vol) + 方向性 **目标 delta** 对冲回测 ([[TargetDeltaHedgeStrategy]])。
  ...
  *
  * 运行: sbt "runMain strategy.targetdeltahedge.backtest.ShortVolHedgeBacktest [start] [end] [fee] [tilt] [band] [market|limit] [iv]
