package strategy.trendride.backtest
...
import strategy.trendride.logic.TrendRideStrategy
...
/** 一次性把两周期的缓存 trades 聚合成 1 分钟 bar 落盘 (供 [[TrendRideSweep]] 快速回放)。
  * 运行: sbt "runMain strategy.trendride.backtest.TrendRideAggregate" (约 10+ 分钟, 只需跑一次)。 */
...
/** TrendRide 参数/框架扫描 —— 在内存分钟 bar 上快速回放多套配置，按两周期表现排序。
  *
  * 运行: sbt "runMain strategy.trendride.backtest.TrendRideSweep" (依赖 TrendRideAggregate 已生成 bar CSV)。
  */
