...
import strategy.bandhedge.logic.{BandHedgeStrategy, CompositeBand, DirectionalBand, MaSideBand, SymmetricAtrBand, VolRegimeBand}
...
/** 买方 edge 月度滚动回测——在已缓存的整段时间上逐月回测，对照基线与各 edge 变体。
  ...
  *
  * 运行: sbt "runMain strategy.bandhedge.backtest.EdgeRollingBacktest [trailing|fair|inflated:1.2] [startMonth yyyy-MM] [endMonth yyyy-MM]"
  */
