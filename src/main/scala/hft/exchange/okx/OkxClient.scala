...
final class OkxClient(
    ...
    restBase: String = OkxClient.RestBaseUrl,
  ...
  /** 供账户流构建 WS 登录帧使用 */
  def wsCredentials: Option[OkxCredentials] = credentials
  ...
  override def placeOrder(order: Order): Either[ExchangeError, OrderId] =
    ...
    val (ordType, pxField) = order.orderType match
      ...
      case OrderType.Limit(price, tif) => (tifToOrdType(tif), s""","px":"${fmt(price)}"""")
    ...
    signedRequest[BalanceResp](Method.GET, "/api/v5/account/balance").flatMap { r =>
      ensureOk(r.code, r.msg).flatMap { _ =>
        r.data.headOption
          ...
          .map(b => AccountInfo(equity = b.totalEq.asDouble, notional = 0.0))
      }
    }

  /** OKX 初始持仓由私有 WS 登录后下发 snapshot，REST 不查询：显式返回空表态 (非沉默漏推) */
  override def fetchPositions(): Either[ExchangeError, Vector[Position]] =
    if hasCredentials then Right(Vector.empty)
    else Left(ExchangeError.Auth("OKX credentials required"))
  ...
  private def publicGet[T: JsonValueCodec](path: String): Either[ExchangeError, T] =
    request(Method.GET, path, body = None, signed = false).flatMap(parse[T])
  ...
  private def signedRequest[T: JsonValueCodec](
    ...
    request(method, path, if method == Method.GET then None else Some(body), signed = true).flatMap(parse[T])
  ...
  private def ensureOk(code: String, msg: String): Either[ExchangeError, Unit] =
  ...
  private def sideFromOkx(side: String): Side = side match
    case "buy"  => Side.Long
    case "sell" => Side.Short
    case other  => throw IllegalStateException(s"Unknown OKX side: '$other'")

  private def tifToOrdType(tif: TimeInForce): String = tif match
