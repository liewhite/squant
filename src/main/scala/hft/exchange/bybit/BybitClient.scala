...
object BybitClient:
  ...
  private val InstrumentsPageLimit = 1000
...
final class BybitClient(
    ...
    restBase: String = BybitClient.RestBaseUrl,
  ...
  /** 供账户流构建 WS auth 帧使用 */
  def wsCredentials: Option[BybitCredentials] = credentials
  ...
  override def placeOrder(order: Order): Either[ExchangeError, OrderId] =
    val (ordType, pxField) = order.orderType match
      ...
      case OrderType.Limit(price, tif) =>
        ("Limit", s""","price":"${fmt(price)}","timeInForce":"${tifToParam(tif)}"""")
  ...
  override def fetchPositions(): Either[ExchangeError, Vector[Position]] =
    if !hasCredentials then Left(ExchangeError.Auth("Bybit credentials required"))
    else
          ...
          resp.result.list.iterator.flatMap { d =>
            fromBybit(d.symbol).map { sym =>
              ...
              Position(
                ...
                symbol = sym,
                size = signedSize,
                entryPrice = d.avgPrice.asDoubleOrZero,
  ...
  private def publicGet[T: JsonValueCodec](path: String, query: String): Either[ExchangeError, T] =
    request(Method.GET, path, query, body = "", signed = false).flatMap(parse[T])

  private def signedGet[T: JsonValueCodec](path: String, query: String): Either[ExchangeError, T] =
    request(Method.GET, path, query, body = "", signed = true).flatMap(parse[T])

  private def signedPost[T: JsonValueCodec](path: String, body: String): Either[ExchangeError, T] =
    request(Method.POST, path, query = "", body = body, signed = true).flatMap(parse[T])
  ...
  private def request(
    ...
    val headersE: Either[ExchangeError, Map[String, String]] =
      if !signed then Right(if method == Method.GET then Map.empty else Map("Content-Type" -> "application/json"))
      else
        credentials.toRight(ExchangeError.Auth("Bybit credentials required")).map { c =>
          ...
          if method == Method.GET then base else base + ("Content-Type" -> "application/json")
        }
    val url = if query.nonEmpty then s"$restBase$path?$query" else s"$restBase$path"
    headersE.flatMap(headers => send(method, url, headers, if method == Method.GET then None else Some(body)))
  ...
  private def ensureOk(retCode: Int, retMsg: String): Either[ExchangeError, Unit] =
