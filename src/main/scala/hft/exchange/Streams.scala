...
trait MarketDataStream:
  ...
  /** 启动行情连接，在给定并发作用域内 fork 出常驻虚拟线程，事件发布到 eventBus */
  def start(eventBus: EventBus)(using Ox): Unit

  /** 订阅公共行情。可在任意时刻调用 (须在 start 之后) */
  def subscribe(kinds: Set[SubscriptionKind]): Unit

/** 私有账户流：推送订单回报、成交、仓位、余额等账户数据到 事件总线。
  *
...
trait AccountStream:
  ...
  /** 启动账户流，在给定并发作用域内 fork 出常驻虚拟线程，事件发布到 eventBus */
  def start(eventBus: EventBus)(using Ox): Unit
