...
/** 柜台插件：**一个账户在一个交易所上的执行与汇报**。
  ...
  *
  * **契约只有一条：账本一变，[[Topics.Position]] 先于其他回报发出。**
  *
  * 于是策略在任何回调里读到的仓位都是最新的。这条是**构造出来的**，不是靠等各家推送
  * 对齐等出来的：柜台先记账、立刻产出仓位快照，再发成交与订单状态。三种柜台
  * (真实 / 替身 / 影子) 与回测都如此。
  *
  * 反过来，本契约**不**承诺订单状态相对成交的位置。Bybit 的成交与订单状态来自两条
  * 独立频道，交易所本身就不保证它们的先后 —— 那是物理事实，装作能消除只会把不确定性
  * 藏起来。**要对仓位变化做反应就订 [[Topics.Position]]**，别在订单回调里读仓位。
  *
  * ## 没有凭证就不装柜台
  *
