...
/** 虚拟柜台的纯核心：撮合 + 延迟决策。
  ...
  *
  * 什么**没**收进来：orderId 生成。三个驱动的差异在并发上下文而非格式 —— 实盘替身的
  * `placeOrder` 在 REST 调用线程上被调，必须用 `AtomicLong`；回测与影子盘都在单线程里推进，
  * 一个 `var` 就够，回测还额外要求它确定性自增才能复现。各三行、机制与驱动绑定，
  * 收进核心只会逼出一个"把 id 生成器传进来"的参数，不会让任何一处更简单。
  */
object Counter:
