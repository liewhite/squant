...
/** 永续 delta 对冲实盘启动器 (引擎集成版)。**复用实盘引擎**: BBO 走引擎已有的 BybitMarketStream (不二次订阅),
  ...
  *
  * 启动时用历史 K 线 prewarm ATR/均线, 避免冷启动等数十小时。**dry-run 默认** (Engine dryRun, 只打日志不下单);
  * `VOLHEDGE_LIVE=1`+key 才实盘。`VOLHEDGE_TESTNET=1` 走 testnet。实盘前务必 testnet 验证。
  *
  * 运行: VOLHEDGE_LIVE=1 BYBIT_API_KEY=.. BYBIT_API_SECRET=.. sbt "runMain voltrade.PerpHedgeEngineLauncher"
  */
@main def PerpHedgeEngineLauncher(): Unit =
  ...
  if credentials.isEmpty then
    logger.error("缺 BYBIT_API_KEY/SECRET (读期权/永续持仓需签名), 退出")
    sys.exit(1)

  logger.warn(s"PerpHedge(引擎集成) 启动: symbol=$symbol ccy=$ccy greeks轮询=${greeksPollMs}ms 带=均线上 上${tightAtr}ATR/下${looseAtr}ATR")
  logger.warn(s"模式: ${if live then "*** 实盘 LIVE ***" else "dry-run (引擎不下单)"}  ${if testnet then "testnet" else "mainnet"}")
