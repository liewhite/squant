...
object VolSell:
  ...
  final case class Decision(mult: Double, rvPrev: Double, rvThis: Double, spot: Double, expiryMs: Long, legs: Seq[Leg])
  ...
  def plan(ex: OptionsExchange, cfg: Config, nowMs: Long): Either[String, Decision] =
    for
      ...
      straddle <- SellVolPlan.selectStraddle(chain, nowMs, spot, cfg.targetDays).toRight("未找到 ~目标天数 ATM 跨式")
      (call, put) = straddle
