...
**不建模资金费**：账本只记成交与手续费，永续资金费一分不收也一分不付（结算周期各家不同：
币安 8 小时、Hyperliquid 1 小时）。
对持仓过夜的方向策略，这一项常常就是全部盈亏的量级；对跨所价差与 delta 对冲，两条腿的
资金费方向相反、净额才是真实收益。不补"典型费率"——`Topics.FundingRate` 上有逐个结算时点的
真实数据，拍一个平均值等于往结论里掺一个没有依据的数。在按真实费率序列结算之前，
**这里的盈亏不可对账**，也别用它去比较持仓时长差异很大的两个策略（被忽略的那一项对两边并不对称）。
...
#### 策略怎么认出自己的回报：`Order.tag`

`clientOrderId` 由框架在处理器**返回之后**才生成（上面第 2 步），所以策略手里那个是空串，
拿它去匹配回报一条都匹配不上。策略要认单，靠的是下单时自己给的标注：

```scala
ctx.place(Order(..., tag = "rich"), "cross_arb_open")   // 下单时标
// ...该单的回报到达时:
.own(Topics.OrderUpdate) { (update, ctx, _) => ctx.orderTag  // Some("rich")
```

框架只负责**原样带住**它：进挂单登记，回报到达时经 `StrategyContext.orderTag` 交还。三条边界：

- **取标注发生在应用事件之前**。订单进终态时挂单登记就被移除，而策略恰恰在终态里做事
  （结算、重挂、平腿）。顺序封在 `StrategyRunner.observe` 一个方法里，不留给调用方记 ——
  写反的症状是"标注恒为 None"，没有报错，只是策略从此认不出自己的单。
- **不出本地**：它不进 `ExchangeOrder`、不发给交易所。所以重启后从交易所接管的既有挂单
  没有标注（那是上一个进程内存里的事实），策略按标注分派时必须能处理这一支 —— 接管单
  一直是这样，与本机制无关。
- **`Fill` 上没有**：`Fill` 载荷不带订单身份，框架无从知道它属于哪一张单。

没有它的时候，三个业务策略各自发明了一套按属性认单的办法（按 `reduceOnly` 分槽、按
(交易所, 标的) 认领、立"同时刻只有一张在途单"的不变量）。都能用，但把"同一标的上同时挂
多张可区分的单"排除在了能写出来的东西之外。
